...
def insert_file(cursor, conn, file_type, header_first, header_second, import_table, export_table):
    cursor.execute('''
...
def insert_elf(cursor, conn, file_path):
...
def add_file(file_path):
    ...
    try:
        ...
        if file_type == 'application/x-dosexec':
            file_id = insert_exe(cursor,conn,file_path)
        elif file_type == 'application/x-executable':
            file_id = insert_elf(cursor,conn,file_path)
        conn.close()
        return file_id
